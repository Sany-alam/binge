...
<div class="container-fluid">
    ...
    <div class="row">
        <div class="col-md-10 col-sm-10 col-10 offset-md-1 offset-sm-1 main_page_new_order">
           ...
           <form action="{{route('show_bp_report')}}" method="post">
           @csrf
               <div class="row">
                   <div class="col-md-3 p-0 m-0">
                    <label for="fullname" class="">From Date</label>
                    <label for="username" class="">To Date</label>
                    <label for="password" class="">Brand Promoter</label>
                   ...
                   <select style="padding: 0px 20px 0 10px;" name="brand_promoter" class="form-control" id="exampleFormControlSelect1">
                        <option value="All">All</option>
                        @foreach($brand_promoters as $brand_promoter)
                        <option value="{{$brand_promoter->id}}">{{$brand_promoter->name}}</option>
                        @endforeach
