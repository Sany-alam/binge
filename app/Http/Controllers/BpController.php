...
class BpController extends Controller
{
    ...
    public function submit_order(Request $request)
    {
        ...
        $request['brand_promoter_id'] = 1;//Auth;
        brand_promoter::create($request->all());
        return redirect()->route('order-generate-bp')->with('success','Order Created Successfully');
    ...
    {   
        ...
        $order = brand_promoter::latest()->first();
        if($order)
        {
            $order_no = $order->id;
        }
        ...
        return view('bp.order_generate',['source_of_leads'=>$source_of_lead,'order_no'=>$order_no+1]);
    }
